...
const Languages: FC = () => {
    const {
        metafield: {languages, setting: {advancedSetting}},
        onClick,
        doLanguageChange,
        language
    ...
    return <div className='behavior-wrap'>
                ...
                <RenderIf cond={config.language_list.length > 0}>
                    <select
                        ...
                        {config.language_list.map((item: string) => {
                            ...
                            return <option key={item} value={item}>{lang?.label}</option>
                        })}
                    </select>
                </RenderIf>
            </div>
        </RenderIf>
        <RenderIf cond={isTrue(advancedSetting.show_close_icon)}>
            <span onClick={() => onClick('close')} className="close-icon">✕</span>
        </RenderIf>
    </div>
}
